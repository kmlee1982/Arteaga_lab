...
library(DESeq2)
library(dplyr)
MyCountData <- read.csv("rawcounts_valencia.csv",header=TRUE)
MyCountData %>% distinct(SYMBOL, .keep_all = TRUE) #Remove duplicated gene
row.names(MyCountData) <- MyCountData$SYMBOL
MyCountData <- MyCountData[,-c(1)]
CountData=as.matrix(MyCountData) 
